...
# Determine number of observations in minority class
minority_class_size <- sum(data$y == "yes")
minority_class_size
# Randomly select a subset of the majority class to remove
majority_class_subset <- data[data$y == "no" & runif(nrow(data)) < minority_class_size / nrow(data), ]
# Combine the majority class subset with the minority class
balanced_data <- rbind(majority_class_subset, data[data$y == "yes", ])
...
count<-c(454,296,711,1328,4,721,1857,259)
...
ggplot(balanced_data, aes(x = nr.employed)) +
  geom_histogram(fill = "turquoise",
                 color = "turquoise4",
                 bins = 40) + # Bins means no of bars
...
balanced_data$id <- as.numeric(balanced_data$id)
...
ggplot(balanced_data,
       aes(x = contact,
           fill = y)) +
  geom_bar(position = "dodge")+
  labs(title = "Response variable vs Contact Variable")
...
ggplot(balanced_data,
       aes(x = education,
           fill = y)) +
  geom_bar(position = "dodge")+
  labs(title = "Response variable vs Education Variable")
...
quanti
str(quanti)
balanced_data$pdays <- as.numeric(balanced_data$pdays)
...
library(ggplot2)
ggplot2::ggplot(data = reshape2::melt(cor_mat)) +
...
nrow(pca_data) #Total data -5689
nrow(train_data) #Train data -3984
nrow(test_data) #Test data -1705
...
# Next, calculate the true positive rate and false positive rate at different probability thresholds
library(pROC)
